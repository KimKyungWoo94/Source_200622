...
void V2X_OBU_ProcessRxMpdu(const uint8_t *const mpdu, const uint16_t mpdu_size)
{
	...
	if (g_dbg >= kDbgMsgLevel_event) {
		...
		//printf("    tx_chan_num: %d, tx_datarate: %d, tx_power: %d, rx_power : %d, RCPI : %d,  priority: %d, psid: %d\n",
		//		dot3_params.tx_chan_num, dot3_params.tx_datarate, dot3_params.tx_power, g_mib.rxpower, g_mib.rcpi, dot3_params.priority, dot3_params.psid);
	...
	if (dot3_params.psid == g_mib.psid) {
		...
		//parsendMQ(outbuf,payload_size);
		if (g_dbg >= kDbgMsgLevel_event) {
			//printf("Processing interseted WSM for PAR  psid %u\n", dot3_params.psid);
			//printf("------------------------------------------------------------\n\n");
			syslog(LOG_INFO | LOG_LOCAL0, "Processing interseted WSM for psid %u\n", dot3_params.psid);
			syslog(LOG_INFO | LOG_LOCAL0, "------------------------------------------------------------\n\n");
		}
