...
/* 전역변수 */
mqd_t recvFD, sendFD, parFD;
struct msgQ_elem_frame *recvPkt = NULL; // 메시지 버퍼
struct msgQ_elem_frame *parPkt = NULL; //kkw 메시지 버퍼
...
int initMQ(void)
{
	if(g_mib.op == opRX || g_mib.op == opTRX)
	{
		...
		/*  PAR 수신 메세지 큐용 버퍼 Allocation */
		parPkt = (struct msgQ_elem_frame *)calloc(1, sizeof(struct msgQ_elem_frame));
		if( parPkt == NULL )
		{
			//printf("[prcsWSM] Fail to allocate memory for receive packet message queue.\n");
			syslog(LOG_ERR | LOG_LOCAL1, "[prcsWSM] Fail to allocate memory for receive packet message queue.\n");
		...
		/* PAR 수신 Packet 메시지큐 생성 */
		parFD = msgget((key_t)KEY_SEND_PAR, IPC_CREAT | 0666);
		if (parFD < 0)
		{
			//perror("[prcsWSM] msgget error ");
			syslog(LOG_ERR | LOG_LOCAL1, "[prcsWSM] msgget error : %s", strerror(errno));
...
int recvMQ(char *pkt)
{
	...
	{
		cnt++;
			//printf("[prcsWSM] %dth MQ receive(len: %d)\n",cnt, sendPkt->msg.msg_len);
		if (g_dbg >= kDbgMsgLevel_event)
		{
			//printf("[prcsWSM] %dth MQ receive(len: %d)\n",cnt, sendPkt->msg.msg_len);
			syslog(LOG_INFO | LOG_LOCAL0, "[prcsWSM] %dth MQ receive(len: %d)\n",cnt, sendPkt->msg.msg_len);
		}
...
{
	static int cnt = 0;
	memset(parPkt->msg.msg, 0, parPkt->msg.msg_len);
	parPkt->msg.msg_len = len;
	memcpy(parPkt->msg.msg, pPkt, len);

	parPkt->rxCnt = msgqCnt++;
	parPkt->msgtype = 1;

	if( msgsnd( parFD, (char *)parPkt, sizeof(struct msgQ_elem_frame) - sizeof(long), IPC_NOWAIT) == -1 )
	{
	...
	{
		cnt++;
		//printf("[prcsWSM] %dth MQ SEND(len: %d)\n",cnt, parPkt->msg.msg_len);
		if (g_dbg >= kDbgMsgLevel_event)
		{
			//printf("[prcsWSM] %dth PAR MQ send(%d Byte) \n",cnt, parPkt->msg.msg_len);
			syslog(LOG_INFO | LOG_LOCAL0, "[prcsWSM] %dth PAR MQ send(%d Byte) \n", cnt, parPkt->msg.msg_len);
		}
	}
}
