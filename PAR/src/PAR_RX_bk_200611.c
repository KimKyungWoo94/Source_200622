...
void getCalDataRcpi(uint8_t* array, int arrayIdx);
void getCalDataRxpower(int16_t* array, int arrayIdx);
...
void par_RXoperation(){
	...
	while(!ending){
		...
		else if(len >0)
		{
			...
			{
#if 1
				if(g_Packet.rsuID > ListPtr->numOfList){
					createNode(/*L*/);
				...
				ListPtr->cur->rxpower[ListPtr->tail->arrayIdx] = g_Packet.rxPower;
				ListPtr->cur->rcpi[ListPtr->tail->arrayIdx] = g_Packet.rcpi;
 ...
 */
void freeAllNode(){
	struct parInfo_t* tempPtr = ListPtr->head;
	while(tempPtr != NULL){
		ListPtr->head = ListPtr->head->next;
		free(tempPtr);
		tempPtr = ListPtr->head;
	}
...
void getCalDataRcpi(uint8_t* array, int arrayIdx){
	...
	syslog(LOG_INFO | LOG_LOCAL2, "Max_rcpi: %d, Min_rcpi: %d, Avr_rcpi: %.0f, Last_rcpi: %d\n",
...
void getCalDataRxpower(int16_t* array, int arrayIdx){
	...
	syslog(LOG_INFO | LOG_LOCAL2, "Max_rxpower: %d, Min_rxpower: %d, Avr_rxpower: %.0f, Last_rxpower: %d\n",
		max, min, (double)sum/arrayIdx, array[arrayIdx-1]);
}
...
void par_Report(void){
	...
	for(idx=0;idx<=g_mib.rsuNum;idx++)
	{
		...
		/* RSUID 일치 여부 */
		if(ptrTemp->rsuID >0 && ptrTemp->rsuID <= g_mib.rsuNum)
		{
			...
			if(g_mib.dbg){
				...
				syslog(LOG_INFO | LOG_LOCAL2, /*"RXPOWER : %d, rcpi : %d,*/"Distance : %.0f, OBUSpeed : %3.2f, OBUHeading : %3.2f, CNT : %u, PAR : %d\n",
						...
						cnt[idx],
						ptrTemp->maxPAR);
				getCalDataRcpi(ptrTemp->rcpi, ptrTemp->arrayIdx);
				getCalDataRxpower(ptrTemp->rxpower, ptrTemp->arrayIdx);
