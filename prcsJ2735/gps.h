...
{
    uint32_t itow;      // GPS time of week of the navigation epoch(uint : ms)
    uint16_t year;      // UTC time
    uint8_t  month;     // UTC time
    uint8_t  day;       // UTC time
    uint8_t  hour;      // UTC time
    uint8_t  min;       // UTC time
    uint8_t  sec;       // UTC time
    uint8_t  valid;     // 16진수, Validity flags
    uint32_t tAcc;      // Time accuracy estimate, UTC time (unit : ns)
    int32_t  nano;      // Fraction of second, UTC time(unit : ns)
    uint8_t  fixType;   // GNSSfix Type
    uint8_t  flags;     // 16진수, Fix status flags
    uint8_t  flags2;    // 16진수, Additional flags
    uint8_t  numSV;     // Number of satellites used in Nav solution
    int32_t  lon;       // Longitude (unit : degree)
    int32_t  lat;       // Latitude (unit : degree)
    int32_t  height;    // Height above ellipsoid (unit : mm)
    int32_t  hMSL;      // Height above mean sea level (unit : mm)
    uint32_t hAcc;      // Horizontal accuracy estimate (unit : mm)
    uint32_t vAcc;      // Vertical accuracy estimate (unit : mm)
    int32_t  velN;      // NED north velocity (unit : mm/s)
    int32_t  velE;      // NED east velocity (unit : mm/s)
    int32_t  velD;      // NED down velocity (unit : mm/s)
    int32_t  gSpeed;    // Ground Speed (unit : mm/s)
    int32_t  headMot;   // Heading of motion (unit : degree)
    uint32_t sAcc;      // Speed accuracy estimate (unit : mm/s)
    uint32_t headAcc;   // Heading accuracy estimate (unit : degree)
    uint16_t pDOP;      // Position DOP
 //   uint8_t  flag3;      // Position DOP
 //   uint8_t  reserved1[5]; // reserved
    uint8_t  reserved1; // reserved
    int32_t  headVeh;   // Heading of vehicle (unit : degree)
    int16_t  msgDec;    // Magnetic declination ( unit : degree)
    uint16_t msgAcc;    // Magnetic declination accuracy ( unit : degree)
};
...
#define UBX_PVT_SET	(1llu<<36) /* yslee */
...
#define STATUS_DGPS_FIX	2	/* yes, with DGPS */
    /* 190828 - yslee */
#define STATUS_FLOAT_RTK	5	
#define STATUS_FIX_RTK	4	
